...
# Create a data frame containing the Decimal Degrees coordinates for the tank_polyline
tank_polyline_data <- data.frame(X = x_decimal, Y = y_decimal)
tank_polyline_data$point <- 1:nrow(tank_polyline_data)

# Sort the data frame based on the "point" column
tank_polyline_data_ordered <- tank_polyline_data[order(tank_polyline_data$point), ]

# Convert the data frame to a LINESTRING spatial object using sf
tank_polyline_sf <- st_sfc(st_linestring(as.matrix(tank_polyline_data_ordered[, c("X", "Y")]), dim = "XY"), crs = 4326)

# Create a buffer_tank around the line (10 meters)
buffer_tank_size <- 20  # 10 meters
buffer_tank_sf <- st_buffer(tank_polyline_sf, dist = buffer_tank_size)
plot(buffer_tank_sf)
st_as_sf(buffer_tank_sf)
class(buffer_tank_sf)
...
# Calculate the centroid of the polygon
tank_centroid <- st_centroid(spdf_tank)
...
MainPasture<- ggplot() +
  ...
  geom_text(data = tank_centroid_df, aes(x = X, y = Y, label = "tank"), color = "black", size = 2) +
...
# Replace these values with your DMS coordinates for the five points
lon_deg <- c(96, 96, 96, 96, 96, 96, 96, 96)
lon_min <- c(40, 39, 39, 39, 39, 39, 39, 39)
lon_sec <- c(0.15, 58.84, 56.88, 54.71,54.68, 57.09, 58.77, 59.68) 
lon_dir <- rep("W", 8)  # West (use "E" for East)

lat_deg <- c(30, 30, 30, 30, 30, 30, 30, 30)
lat_min <- c(47, 47, 47, 47, 47, 47, 47, 47)
lat_sec <- c(32.62, 30.79, 31.01, 32.65, 34.92, 32.31, 32.49, 32.96)
lat_dir <- rep("N", 8)  # North (use "S" for South)
...
# Create a dataframe for lot' coordinates
# Create an sf polygon object from the five points (assumes a closed polygon)
tank <- as.data.frame(list(rbind(c(lon_dd[1], lat_dd[1]),  # Point 1
                                         c(lon_dd[2], lat_dd[2]),  # Point 2
                                         c(lon_dd[3], lat_dd[3]),  # Point 3
                                         c(lon_dd[4], lat_dd[4]),  # Point 4
                                         c(lon_dd[5], lat_dd[5]),  # Point 5
...
tank_Poly <- Polygon(tank)
tank_Poly_ps <- Polygons(list(tank_Poly),1)
tank_Poly_sps <- SpatialPolygons(list(tank_Poly_ps))
proj4string(tank_Poly_sps) <- CRS("+proj=longlat +datum=WGS84 +no_defs +ellps=WGS84 +towgs84=0,0,0")
data = data.frame(f=99.9)
spdf_tank = SpatialPolygonsDataFrame(tank_Poly_sps,data)
spdf_tank
summary(spdf_tank)

# Set as simple feature
spdf_tank <- st_as_sf(spdf_tank)
spdf_tank <- st_transform(spdf_tank, st_crs(sampling_area))
spdf_tank <- st_as_sf(spdf_tank)
...
# Replace these values with your DMS coordinates for the five points
lon_deg <- c(96, 96, 96, 96)
lon_min <- c(39, 39, 39, 39)
lon_sec <- c(55.83, 56.26, 56.13, 55.68) 
lon_dir <- rep("W", 4)  # West (use "E" for East)

lat_deg <- c(30, 30, 30, 30)
lat_min <- c(47, 47, 47, 47)
lat_sec <- c(29.86, 29.68, 29.43, 29.57)
lat_dir <- rep("N", 4)  # North (use "S" for South)
...
# Create a dataframe for lot' coordinates
# Create an sf polygon object from the five points (assumes a closed polygon)
barn <- as.data.frame(list(rbind(c(lon_dd[1], lat_dd[1]),  # Point 1
                                         c(lon_dd[2], lat_dd[2]),  # Point 2
                                         c(lon_dd[3], lat_dd[3]),  # Point 3
                                         c(lon_dd[4], lat_dd[4]),  # Point 4
                                         c(lon_dd[1], lat_dd[1]))))

barn_Poly <- Polygon(barn)
barn_Poly_ps <- Polygons(list(barn_Poly),1)
barn_Poly_sps <- SpatialPolygons(list(barn_Poly_ps))
proj4string(barn_Poly_sps) <- CRS("+proj=longlat +datum=WGS84 +no_defs +ellps=WGS84 +towgs84=0,0,0")
data = data.frame(f=99.9)
spdf_barn = SpatialPolygonsDataFrame(barn_Poly_sps,data)
spdf_barn
summary(spdf_barn)

# Set as simple feature
spdf_barn <- st_as_sf(spdf_barn)
spdf_barn <- st_transform(spdf_barn, st_crs(sampling_area))
spdf_barn <- st_as_sf(spdf_barn)
...
# Step 7: Save the raster layer to a Geotiff file
writeRaster(raster_clip, filename = "raster_Gause_prop.tif", format = "GTiff", overwrite= TRUE)

# Step 8; Load DEM tiff
elevation_gause <- raster("raster_Gause_prop.tif")

# Convert the raster data to a data.frame
elevation_gause_df <- raster::rasterToPoints(elevation_gause)
elevation_gause_df <- as.data.frame(elevation_gause_df)
plot(raster_clip)
...
sampling_area_excluded_ponds12 <- st_difference(sampling_area_excluded_ponds1, spdf_tank)
plot(sampling_area_excluded_ponds12)

sampling_area_excluded_all<- st_difference(sampling_area_excluded_ponds12, spdf_barn)
plot(sampling_area_excluded_all)
...
#Create .csv file with sample points, coordinates, and number
GauseSamplingPoints <- write.csv(sampling_points_df, "GauseSamplingPoints.csv")

##########################################################################################################
# Crop 'polygon_to_crop' using 'cropping_polygon'
cropped_tank <- st_intersection(spdf_tank, sampling_area)
cropped_tank <- st_as_sf(cropped_tank)
...
#Pond 2
tank_centroid_2 <- st_centroid(spdf_tank)
...
#Barn
barn_centroid_3 <- st_centroid(spdf_barn)

barn_centroid_3_3 <- st_coordinates(barn_centroid_3) %>%
  as.data.frame() %>%
  setNames(c("X", "Y"))
...
# Plot the shapefile with ggplot2 and add the raster data as a background layer
# Manually specify the three specific values for the breaks on the x-axis (DMS format)
breaks_x <- c(-96.668, -96.666, -96.664, -96.662)

Gause<- ggplot() +
  geom_raster(data = elevation_gause_df, aes(x = x, y = y, fill = raster_Gause_prop*3.28084)) +    # Plot the raster data
  geom_sf(data = spdf_pond_pond1, fill = "lightblue") +
  geom_text(data = tank_centroid_df_1, aes(x = X, y = Y, label = "TANK"), color = "black", size = 2) +
  geom_sf(data = cropped_tank, fill = "lightblue") +
  geom_text(data = tank_centroid_df_2, aes(x = X, y = Y, label = "TANK"), color = "black", size = 2, nudge_y = -0.0001) +
  geom_sf(data = spdf_barn, fill = "orange") +
  geom_text(data = barn_centroid_3_3, aes(x = X, y = Y, label = "BARN"), color = "black", size = 2, nudge_y = 0.00015) +
  geom_sf(data = sampling_area, color = "black", fill = "transparent") +       # Plot the shapefile
  geom_point(data = sampling_points_df, aes(x = X, y = Y), color = "red", size = 2) +  # Plot the final sampling points
  geom_text(data = sampling_points_df, aes(x = X, y = Y, label = Sample_Number),
            color = "black", nudge_x = -0.00015, nudge_y=-0.00015) +                # Adding labels to the sampling points
  scale_fill_viridis_c() +                                                              # Use Viridis color scale (you can change this)
  coord_sf(crs = st_crs(sampling_area)) +                                     # Set the coordinate system for the plot
  labs(x = "Longitude", y = "Latitude", fill = "Elevation (ft)", title = "Gause DEM w/ Sampling Locations") + # Add the following line to set the desired breaks on the x-axis
  scale_x_continuous(labels = function(x) decimal_to_dms_with_directions(x, "x"), expand = c(0, 0), breaks =  breaks_x) +
  scale_y_continuous(labels = function(y) decimal_to_dms_with_directions(y, "y"), expand = c(0, 0)) +  # Use custom labeling for Y-axis
  theme(axis.title.x = element_text(size = 12, color = "black"),
        axis.text.x = element_text(size = 7, color = "black"),
        axis.title.y = element_text(size = 12, color = "black"),
...
ggsave("DEM_Sampling_Gause.jpeg", Gause ,height = 7, width = 7, dpi = 300)
```
